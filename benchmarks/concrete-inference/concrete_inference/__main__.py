...
def calc_cost(model_name, input_tokens, output_tokens):
    ...
    """
    cost = MODEL_COST_PER_INPUT[model_name] * input_tokens + MODEL_COST_PER_OUTPUT[model_name] * output_tokens
...
# region concrete
def concrete_operator_inference(
    test_dataset,
    model_name_or_path,
    output_file,
    model_args,
    ...
    """
    Runs inference on a dataset using a concrete operator
    Simple tester to verify concrete integration. Zero-shot.
    ...
    with open(output_file, "a+") as f:
        for datum in tqdm(test_dataset, desc=f"Inference for {model_name_or_path}"):
            ...
            response = operator.chat(
                output_dict["text"], options={"response_format": concrete.models.messages.TextMessage}
            )
...
def main(
    ...
    elif model_name_or_path.startswith('concrete'):
        concrete_operator_inference(**inference_args)
    else:
